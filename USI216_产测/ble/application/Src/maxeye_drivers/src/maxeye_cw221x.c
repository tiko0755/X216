...
int32_t cw221x_init(void)
{
    ...
    if(ret!=0)
    {
        logX("</%s auto_discern_ic_addr>", __func__);
        return ret;
    }
    ...
    if (ret < 0)
    {
        logX("</%s cw221x_get_state>", __func__);
        return ret;
    }
    ...
    if (ret != 0)
    {
        ...
        if (ret < 0)
        {
            logX("</%s cw221x_config_start_ic>", __func__);
            return ret;
        }
    }
    logX("</%s cw221x_init_success>", __func__);
    return 0;
}
...
int32_t cw221x_get_capacity(uint32_t *lp_uisoc)
{
    ...
    if(ret){
        logX("</%s CW221X_ERROR_IIC >", __func__);
        return CW221X_ERROR_IIC;
    }
    logX("<%s temp_val_buff:0x%04x >", __func__, temp_val_buff);
    soc = temp_val_buff >> 8;
    ...
    else
    {
        ...
        if((UI_SOC >= (cw_bat.capacity - 1)) && (UI_SOC <= (cw_bat.capacity + 1))
            ...
            logX("<%s CAP_DO_NOT_CHANGE>");
        }
    }

    *lp_uisoc = UI_SOC;

    logX("</%s UI_SOC:%d >", __func__, UI_SOC);
    return 0;
}
