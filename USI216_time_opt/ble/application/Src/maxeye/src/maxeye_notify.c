...
void srvc1_battery_charge_notify(void)
{
    ...
    {
        logX("s1 chg ntfy err %x",error_code); 
    }
    else
    {
        logX("s1 chg ntfy %x",notfbuff[2]); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_battery_capacity_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("<%s s1_cap_ntfy_err:0x%x", __func__, error_code); 
        battLevel=BATT_CAP_LEVEL_INVAILD; 
    }
    else
    {
        logX("<%s cw_bat.capacity:%d >",__func__, cw_bat.capacity); 
    }
...
void srvc1_fw_version_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 fwver ntfy err %x",error_code); 
    }
    else
    {
        logX("s1 fwver ntfy"); 
...
void srvc1_hw_version_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 hwver ntfy err %x",error_code); 
    } 
    logX("</%s >", __func__); 
}
...
void srvc1_model_id_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 sn ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_fp24_version_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 fp24 ntfy err %x",error_code); 
    }
     logX("</%s >", __func__); 
}
...
void srvc1_to_sleep_ack_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 sleep ntfy err %x",error_code); 
    }
     logX("</%s >", __func__); 
}
...
void srvc1_rep_voltage_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 vol ntfy err %x",error_code); 
    }
     logX("</%s >", __func__); 
}
...
void srvc1_rep_current_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 current ntfy err %x",error_code);
    }
}
...
void srvc1_rep_temp_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 temp ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_dbclick_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 dbclick ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_ack_notify(uint8_t msgType)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 ack ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_decoding_test_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 decoding test ntfy err %x",error_code); 
    }
...
void srvc1_rep_mmi_flag_notify(void) //半成品测试结果
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 mmi ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_g_sensor_calibration_value_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 g-s cali ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_g_sensor_value_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 g-s value ntfy err %x",error_code); 
...
void srvc1_rep_pressure_value_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 pressure value ntfy err %x",error_code); 
    }
}
...
void srvc1_rep_pressure_test_notify(uint8_t bStatus)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 pressure test ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_pressure_cali_notify(uint8_t bStatus)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 pressure cali ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_disable_preload_notify(uint8_t *pData)
{
    ...
    if (error_code != SDK_SUCCESS)
    {
        logX("s1 dis preload ntfy err %x", error_code);
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_aging_flag_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 aging flag ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_aging_test_result_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 aging result ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
//				}
    ...
    error_code=maxeye_srvc1_notify(0,notfbuff,sizeof(notfbuff));
	logX("aging status111:%d,cnt:%d,time:%d ",wAgingStatus,wAgingCnt,wAging_time_tick); 
	
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 aging test ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_aging_number_notify(void)
{
    ...
    logX("<%s wLen:%d>", __func__,wLen); 
    error_code=maxeye_srvc1_notify(0,notfbuff,sizeof(notfbuff));
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 aging number ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_set_mmi_flag_notify(uint8_t bStatus)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 set mmi flag ntfy err %x",error_code); 
    }
    logX("<%s >", __func__); 
}
...
void srvc1_rep_charge_coefficient_notify(void)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 rep charge para ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_device_check_ack_notify(uint8_t bTimer)
{
    ...
    if(error_code!=SDK_SUCCESS)
    {
        logX("s1 device check ntfy err %x",error_code); 
    }
    logX("</%s >", __func__); 
}
...
void srvc1_rep_pencil_status_notify(uint8_t bStatus)
{
    ...
    if (error_code != SDK_SUCCESS)
    {
        logX("s1 pencil status ntfy err %x--%d", error_code,bStatus);
    }
    logX("</%s >", __func__); 
}
...
static void maxeye_srv_event_handler(void* p_ctx)
{
    ...
    {
        logX("</%s error@app_queue_pop>", __func__); 
        return;
    }
    ...
    switch (buff[0])
    {
        ...
        {
            logX("<%s case:BLE_REQ_CHARGEING_STATUS>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_DB_CLICK_STATUS>", __func__); 
            fgDbClickTest=true;
        }
        break;
        ...
        {
            logX("<%s case:BLE_REQ_FW_VERSION>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_BATTERY_CAP>", __func__);
        ...
        {
            logX("<%s case:BLE_REQ_BATT_CURRENT>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_BATT_TEMP>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_BATT_VOLTAGE>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_FP24_VERSION>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_PENCIL_SLEEP>", __func__); 
            if(buff[1])
            {
        ...
        {
            logX("<%s case:BLE_REQ_HW_VERSION>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_MODEL_ID>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_OPPO_SN>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_MMI_FLAG>", __func__); 
        ...
        {
            logX("<%s case:BLE_SET_MMI_FLAG>", __func__); 
...
//解码
        ...
        {
            logX("<%s case:BLE_REQ_CODING_RATE>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_DECODING_TEST>", __func__); 
            // logX("start decoding");
        ...
        {
            logX("<%s case:BLE_G_SENSOR_CALIBRATION>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_G_SENSOR_CALI_VAL>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_G_SENSOR_VAL>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_PRESSURE_TEST>", __func__); 
        ...
        {
            logX("<%s case:BLE_SET_PRESSURE_GRADE>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_PRESSURE_VAL>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_CHARGE_CODFFICIENT>", __func__); 
        ...
        case BLE_REQ_MMI_SN:
        {
            logX("<%s case:BLE_REQ_MMI_SN>", __func__); 
            srvc1_mmi_sn_notify();
        }
        break;
        ...
        {
            logX("<%s case:BLE_REQ_AGING_FLAG>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_AGING_RESULT>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_SHIPMODE>", __func__); 
        ...
        {
            logX("<%s case:BLE_SET_AGING_TEST>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_AGING_NUMBER>", __func__); 
        ...
        {
            logX("<%s case:BLE_CMD_HALL_CLOSE>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_PENCIL_WDT_RST>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_DEVICE_CHECK>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_DEVICE_CHECK_RESULT>", __func__); 
        ...
        {
            logX("<%s case:BLE_REQ_FILM_VERSION>", __func__); 
        ...
        {
            logX("<%s case:BLE_REP_PENCIL_STATUS>", __func__); 
