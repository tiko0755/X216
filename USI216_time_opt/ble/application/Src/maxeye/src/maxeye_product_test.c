...
{
    logX("<%s wAgingStatus:0x%04>", __func__, wAgingStatus);
    uint8_t buff[4]={0};
    uint16_t wLen;
    char logstr[36];
...
{
    logXX("<%s wAgingCnt:%d>", __func__, wAgingCnt);
    ...
    switch(agingDevicIndex)
    {
        case AGING_DEVICE_BLE:
        {
            ...
            eventInterval = 10;
            logXX("<%s case:AGING_DEVICE_BLE>", __func__);
        }
        break;
...
//            agingDevicIndex=AGING_DEVICE_SLEEP_WAKEUP;
            logXX("<%s case:AGING_DEVICE_AISC>", __func__);
        }
        break;
        ...
        case AGING_DEVICE_SLEEP_WAKEUP:
        {
            ...
            eventInterval=500;
//            agingDevicIndex=AGING_DEVICE_BATT;
            logXX("<%s case:AGING_DEVICE_SLEEP_WAKEUP>", __func__);
        }
        break;
...
//            agingDevicIndex=AGING_DEVICE_P_SENSOR;
            logXX("<%s case:AGING_DEVICE_BATT>", __func__);
        }
        break;     
...
//            agingDevicIndex=AGING_DEVICE_A_SENSOR;
            logXX("<%s case:AGING_DEVICE_P_SENSOR>", __func__);
        }
        break;  
...
//            agingDevicIndex=AGING_DEVICE_CHARGE;
            logXX("<%s case:AGING_DEVICE_A_SENSOR>", __func__);
        }
        break;  
...
//            agingDevicIndex=AGING_DEVICE_FILM;
            logXX("<%s case:AGING_DEVICE_CHARGE>", __func__);
        }
        break;  
...
//            agingDevicIndex=AGING_DEVICE_WLC;
            logXX("<%s case:AGING_DEVICE_FILM>", __func__);
        }
        break;  
...
//            agingDevicIndex=AGING_DEVICE_BATT_METER;
            logXX("<%s case:AGING_DEVICE_WLC>", __func__);
        }
        break;  
...
//            agingDevicIndex=AGING_DEVICE_MCU_RESET;
            logXX("<%s case:AGING_DEVICE_BATT_METER>", __func__);
        }
        break;  
        ...
        case AGING_DEVICE_MCU_RESET:  //MCU 复位脚检测，影响升级
        {
            ...
            eventInterval=2000;      //MCU 启动时间2s，后期优化
            agingDevicIndex=AGING_DEVICE_TOUCH_INT;
            logXX("<%s case:AGING_DEVICE_MCU_RESET>", __func__);
        }
        break;  

        default:
        {
            logXX("<%s case:default>", __func__);
            if(wAgingCnt)
            {
                wAgingCnt--;
                ble_idle_event_start(PENCIL_SLEEP_TIMEOUT_MMI_TEST);
            ...
            logXX("<%s wAgingStatus:%d>", __func__, wAgingStatus);
            logXX("<%s wAgingCnt:%d>", __func__, wAgingCnt);
            logXX("<%s wAging_fail_tick:%d>", __func__, wAging_fail_tick);
            logXX("<%s wAging_fail_tick_mcu_sleep:%d>", __func__, wAging_fail_tick_mcu_sleep);
            logXX("<%s wAging_fail_tick_aisc:%d>", __func__, wAging_fail_tick_aisc);
            logXX("<%s wAging_fail_tick_P_sensor:%d>", __func__, wAging_fail_tick_P_sensor);        
        }
        break;
    }
...
static void maxeye_pcba_event_handler(void* p_ctx) //pcba自检
{
    ...
    logX("<%s wAgingStatus:%d agingDevicIndex:%d>",__func__,wAgingStatus,agingDevicIndex); 
    if(eventInterval)
    {
...
void maxeye_product_test_handler(uint8_t *pData,uint8_t bLen)
{
    ...
    switch(pencil_msg->bCmd)
    {
        ...
        case MAXEYE_CLI_RST_VOLTAMETER:   //复位电量计
        {
            ...
            srvc1_rep_rst_voltameter_notify();
            ble_idle_event_start(PENCIL_SLEEP_TIMEOUT_MMI_TEST);
        }
        break;
