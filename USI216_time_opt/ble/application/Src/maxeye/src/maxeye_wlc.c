...
static uint8_t check_wlc_ack_match(void)
{
    ...
    logXX("<%s ask_ack_err:%02X %02X>", __func__, buf[0], buf[1]);
    return 0;
}
...
static void wlc_int_event_handler(void* p_ctx)
{
    ...
    if (ret_code == APP_DRV_SUCCESS)
    {
        if (int_flag & 0x40)                // ldo enable
        {
            ...
            wlc_ask_task_start(5, NULL);          // 启动ASK数据传递
        }
...
void maxeye_wlc_init(CB2 cmplt)
{
    ...
    {
//        MP2662_EnableCharge();
//        bat_set_chg_cc_to_target(11, 0x03); // 建立轻负载以保证ASK数据稳定传输
        ask_pack_index = 0;
        wlcStatus = WLC_DEV_POWER_UP;
        clear_wlc_int();                    // 清中断以恢复INT电平
        wlc_ask_task_start(10, cmplt);             // 确保在g sensor前
    }
    else
    {
        logX("<%s err@chipid:%04x>", __func__, chipid);
