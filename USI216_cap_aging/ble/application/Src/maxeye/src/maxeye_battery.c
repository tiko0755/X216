...
static CB2 batt_charge_init_cmplt = NULL;
...
sdk_err_t batt_meter_event_start(uint16_t wDelaymS)
{
    ...
    if(ret!=SDK_SUCCESS)
    {
        logX("<%s error@app_timer_start>", __func__);
    }
    logX("</%s >", __func__);
    return ret;
}
...
static void batt_meter_event_handler(void* p_ctx)
{
    ...
    else
    {
        ...
        {
            logX("</%s bleConnStatus==BLE_NO_CONN_NO_ADV %d >", __func__, cw_bat.capacity);
            return;
        }
        #else
        ...
        if (bleConnStatus == BLE_NO_CONN_NO_ADV)    // 未连接状态下 低于阈值或低电中断都会关机 1%-0%关机
        {
            ...
            {
                logX("</%s meter_susp_cap>", __func__);
            }
            return;
        }
 ...
 *****************************************************************************************
 */
sdk_err_t batt_charge_init_event_start(uint16_t wDelaymS, CB2 cmplt)
{
    sdk_err_t     ret;
    batt_charge_init_cmplt = cmplt;
    battChargeInitCnt = 0;
...
{
    // just do the register job
    batt_charge_init_event_register();  // will NOT start timer
    batt_meter_init_event_register();   // will NOT start timer
}
